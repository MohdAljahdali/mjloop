...
1. Call `mjloop_state_get`. If nothing is `running`, say so and stop — there is nothing to halt.
2. Call `mjloop_halt` with the reason. Use $ARGUMENTS when given; otherwise
   `"stopped by the user"`.
3. If the summary named a `story`, that story is still `doing` and `--next` skips it
   forever. Call `mjloop_story_update` on it: `status: "todo"` to put it back in the queue,
   or `status: "blocked"` if the stop reason is something that must be resolved first.
   Leave `evidence` alone — a cancelled run proved nothing. Then call `mjloop_index_render`.
