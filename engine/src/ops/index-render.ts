...
const HEADER = '<!-- generated by mjloop_index_render — do not edit -->'
...
export async function renderIndex(projectDir: string, now: Clock = () => new Date()): Promise<string> {
  ...
  // The lock is a non-recursive mkdir, so `.mjloop` must exist first — an index
  ...
  return withLock(paths.lock, async () => {
    ...
    // `.mjloop` was created before the lock was taken, so the index has a home.
