 ...
 */
function isComplete(plan: PlanView): boolean {
 ...
 */
function sameCycle(a: Snapshot['state']['last_cycle'], b: Snapshot['state']['last_cycle']): boolean {
 ...
 */
export function deriveEvents(previous: Snapshot | null, next: Snapshot): Event[] {
  const events: Event[] = []
  if (previous === null) return events

  const previousStoryStatus = new Map<string, string>()
  ...
  const previousJobStatus = new Map<string, string>(previous.queue.map((job) => [job.id, job.status]))
