...
<template>
  <li class="story detail">
    ...
    <!-- The trailing `{ids: ''}` renders the surrounding phrase alone; the
         identifiers themselves are appended through one `<Bdi>` rather than
         travelling through the translated string as a `{param}` — see the
         migration's own rule on composed strings and identifiers. -->
    <span class="waits" data-slot="waits" :hidden="waiting.length === 0">{{ t('story.blockedBy', { ids: '' }) }}<Bdi :value="waiting.join(', ')" /></span>
