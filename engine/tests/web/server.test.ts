...
/**
 * This suite imports `src/web/server.ts` directly, unbundled, for speed —
 * which is what makes its `PUBLIC_DIR` an issue at all; see
 * `vitest.config.ts`'s `serverPublicDirForTests` for why and how it is
 * repointed at `dist/web/public` for every test in this file, and the
 * precondition on `dist/web/public` actually existing.
 */
