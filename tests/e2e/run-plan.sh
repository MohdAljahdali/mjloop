...
claude -p "/mjloop:plan add a cancelLabel() export to the button module, returning the human label 'Cancel', covered by a test" \
  --permission-mode acceptEdits --allowedTools "${allowed[@]}"

echo "--- plans ---"
find .mjloop/plans -type f | sort

plan_dir="$(find .mjloop/plans -maxdepth 1 -mindepth 1 -type d | head -1)"
...
[[ -f .mjloop/INDEX.md ]] || fail "INDEX.md was not rendered"
